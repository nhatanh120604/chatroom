...
ApplicationWindow {
    ...
    function appendPrivateMessage(peer, author, text, isOutgoing, messageId, status, filePayload, timestamp) {
        ...
        text = text || ""
        if (text.length === 0) {
            // For chunked transfers, check for filename and transfer_id
            // For inline transfers, check for name and data
            if (!filePayload || (!filePayload.filename && !filePayload.name) || (!filePayload.transfer_id && !filePayload.data)) {
                return
            }
        }
        ...
        var fileName = fileInfo && fileInfo.filename ? fileInfo.filename : (fileInfo && fileInfo.name ? fileInfo.name : "")
    ...
    Component {
        ...
        ColumnLayout {
            ...
            Platform.FileDialog {
                ...
                onAccepted: {
                    console.log("[QML] publicFileDialog.onAccepted called")
                    var target = ""
                    if (publicFileDialog.file && publicFileDialog.file.toString) {
                        target = publicFileDialog.file.toString()
                        console.log("[QML] File from publicFileDialog.file.toString():", target)
                    } else if (publicFileDialog.file) {
                        target = String(publicFileDialog.file)
                        console.log("[QML] File from String(publicFileDialog.file):", target)
                        ...
                        console.log("[QML] File from publicFileDialog.files[0]:", target)
                    } else {
                        console.log("[QML] No file selected in publicFileDialog")
                    }
                    
                    if (target.length > 0) {
                        console.log("[QML] Calling chatClient.inspectFile with:", target)
                        try {
                            var meta = chatClient.inspectFile(target)
                            console.log("[QML] inspectFile returned:", JSON.stringify(meta))
                            if (meta && meta.path) {
                                window.publicPendingFile = meta
                                console.log("[QML] Set publicPendingFile:", JSON.stringify(meta))
                                messageField.forceActiveFocus()
                            } else {
                                console.log("[QML] inspectFile failed or returned empty path")
                            }
                        } catch (e) {
                            console.error("[QML] Exception in inspectFile:", e)
                        }
                    } else {
                        console.log("[QML] target is empty")
                    }
                }
            }
    ...
    Component {
        ...
        ColumnLayout {
            ...
            Platform.FileDialog {
                ...
                onAccepted: {
                    console.log("[QML] privateFileDialog.onAccepted called")
                    if (peerKey.length === 0) {
                        console.log("[QML] peerKey is empty, returning")
                        return
                    }
                    var target = ""
                    if (privateFileDialog.file && privateFileDialog.file.toString) {
                        target = privateFileDialog.file.toString()
                        console.log("[QML] File from privateFileDialog.file.toString():", target)
                    } else if (privateFileDialog.file) {
                        target = String(privateFileDialog.file)
                        console.log("[QML] File from String(privateFileDialog.file):", target)
                    } else if (privateFileDialog.files && privateFileDialog.files.length > 0) {
                        var candidate = privateFileDialog.files[0]
                        target = candidate && candidate.toString ? candidate.toString() : String(candidate)
                        console.log("[QML] File from privateFileDialog.files[0]:", target)
                    } else {
                        console.log("[QML] No file selected in privateFileDialog")
                    }
                    ...
                    if (target.length > 0) {
                        ...
                        try {
                            ...
                            if (meta && meta.path) {
                                ...
                                console.log("[QML] Set privatePendingFile for", peerKey)
                                messageField.forceActiveFocus()
                                chatClient.indicatePrivateTyping(peerKey, false)
                                privateTypingTimer.stop()
                            } else {
                        ...
                        Column {
                            ...
                            Rectangle {
                                ...
                                Button {
                                    text: "Download"
                                    focusPolicy: Qt.NoFocus
                                    enabled: model.fileName && model.fileName.length > 0
                                    ...
                                    onClicked: {
                                        ...
                                        if (model.fileData && model.fileData.length > 0) {
                                            console.log("[QML] Downloading inline file:", model.fileName)
    ...
    Connections {
        ...
        function onMessageReceived(username, message, file) {
            ...
            if (file) {
                console.log("[QML] File object keys:", Object.keys(file).join(", "))
            }
        ...
        function onUsersUpdated(users) {
            console.log("[QML] onUsersUpdated called with:", typeof users, users)
            usersModel.clear()
            
            // Defensive check: ensure users is an array
            if (!users || typeof users !== 'object') {
                console.error("[QML] onUsersUpdated: users is not an object:", users)
                window.totalUserCount = 0
                return
            }
            
            // Handle both array and object cases
            var userList = Array.isArray(users) ? users : Object.keys(users)
            window.totalUserCount = userList.length
            
            for (var i = 0; i < userList.length; i++) {
                var username = Array.isArray(users) ? users[i] : userList[i]
                if (username && username !== chatClient.username) {
